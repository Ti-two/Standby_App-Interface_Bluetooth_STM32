...
#ifdef _PROGRAMATION
TIM_HandleTypeDef    	Tim1msReprogHandle;	    	//TIM 7
#endif
TIM_HandleTypeDef    	TimSDDelayHandle;			//TIM 15
TIM_HandleTypeDef    	Tim1msHandle;			    //TIM 16
...
void HWI_Timer_1ms_Init(void)
{
	...
	TIM_MasterConfigTypeDef sMasterConfig = {0};
		...
		Tim1msHandle.Init.RepetitionCounter = 0;
...
TIM_MasterConfigTypeDef sMasterConfig = {0};
	...
	Tim1msReprogHandle.Instance = TIM7;
	Tim1msReprogHandle.Init.Period = uwPeriodValue-1;
	Tim1msReprogHandle.Init.Prescaler = uwPrescalerValue-1;			// incrementation tous les TIMxCLK divis� par uwPrescalerValue
	...
	Tim1msReprogHandle.Init.RepetitionCounter = 0;
	...
	sMasterConfig.MasterOutputTrigger = TIM_TRGO_RESET;
	sMasterConfig.MasterSlaveMode = TIM_MASTERSLAVEMODE_DISABLE;
	if (HAL_TIMEx_MasterConfigSynchronization(&Tim1msReprogHandle, &sMasterConfig) != HAL_OK)
	{
		_Error_Handler(__FILE__, __LINE__);
	}
}
...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef* htim_base)
{
	if(htim_base->Instance==TIM16)
	{
		...
		HAL_NVIC_SetPriority(TIM1_UP_TIM16_IRQn, HAL_NVIC_PREEMPT_PRIORITY_T_1_MS, HAL_NVIC_SUB_PRIORITY_T_1_MS);
  ...
  else if(htim_base->Instance==TIM7)
  {
        ...
        HAL_NVIC_SetPriority(TIM7_IRQn, HAL_NVIC_PREEMPT_PRIORITY_T_1_MS_REPROG, HAL_NVIC_SUB_PRIORITY_T_1_MS_REPROG);
...
void HWI_Timer_Sd_Delay_Init(void)
{
	...
	TimSDDelayHandle.Instance = TIM15;
