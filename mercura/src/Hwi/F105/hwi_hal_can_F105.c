...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef* hcan)
{
	...
	if (hcan->Instance == CAN1)
	{
		if (HAL_CAN_GetRxMessage(&Can1Handle,CAN_RX_FIFO0,&RxMessage, hwi_can_data) == HAL_OK)
		{
            ...
            {
               APP_SendTrame_CAN_ethernet(RxMessage.StdId, hwi_can_data,RxMessage.DLC);
            }
#endif
            if (FLASHAGE_DIAGNOSTIQUE == APP_Flashage_state)
            {
                APP_SendTrame_CAN_wifi(RxMessage.StdId, hwi_can_data,RxMessage.DLC);
            }

            if (TRUE == P_L_CarteSd_Programation_vue)
            {
                P_L_Can_flashage_rx_frame(RxMessage.StdId, hwi_can_data,RxMessage.DLC);
            }
            else
            {
                P_L_Can_1_rx_frame(RxMessage.StdId, hwi_can_data,RxMessage.DLC);
            }
#else
            P_L_Can_1_rx_frame(RxMessage.StdId, hwi_can_data,RxMessage.DLC);
#endif
