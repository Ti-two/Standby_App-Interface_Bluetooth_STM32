...
sub WriteMea()
{
 ...
 print "#*************************************************************************#\n";
  ...
  print "*\n";
  print "*         Script de compilation V1.0 pour facade XXL JAVA \n";
  print "#*************************************************************************#\n";
  print "*\n";
  ...
  $APP_DIRECTORY_HEX = "$ProjectDirectory\\Hex";
  $DOSSIER_OLD = "$ProjectDirectory\\gcm_v1\\";
  
	print "$ProjectDirectory" ;  
	print "\n";
	
	print "* Etape 0 : lecture du fichier old facade dans gcm_v1\n";
	print "\n";
	
# 1-  on recherche le fichier hex V1
	if(! opendir (DossierALire,$DOSSIER_OLD))
	{
		print "* ** Impossible d'ouvrir le repertoire gcm_v1 ** \n";
		system("pause");
		die "*\n#*************************************************************************#\n";  
	}
	
	while(defined(my $fic=readdir DossierALire)){
		my $f="${DossierALire}/$fic";
		if($fic=~/.*\.gcm/){
			print "\n";
			if(! open(OLD_GCM, "<$DOSSIER_OLD$f" ))
			{
				print "* ** Impossible d'ouvrir le fichier old gcm ** \n";
				system("pause");
				die "*\n#*************************************************************************#\n";  
			}
			else
			{
				print "* ** ouverture du fichier old gcm ** \n";
			}
			close(DossierALire);
			last;
		}
...
#Lancement du calcul de checksum du fichier issu de la compilation, puis préparation de la ligne I03
  ...
  if (! open(HEX_FILE, "$Target_Directory\\Facade_XXL_Bsp\\Release\\$HexSourcefile\.hex"))
  {
  ...
  {
	print "* ** ouverture du fichier HEX ** \n";
  }
  $ChecksumDec = 0;
  ...
  #system("pause");
  $Lignetest = "I03  Checksum : ";
  $LignetestFin = "I24";
  while(defined($LigneEnCours = <OLD_GCM>))
  {  
    if ($Lignetest=~substr($LigneEnCours,0,15))
    {
		$ChecksumHexOld = substr($LigneEnCours,16,4);
	}
	if ($LignetestFin=~substr($LigneEnCours,0,3))
    {
		last;
	}
  }
	...
	if (! open(GCM_FILE, "+>$Target_Directory\\gcm_v1v2\\$SW_Name$SW_Version_WithoutPoint\.gcm"))
	{
...
#Préparation de la ligne I03 
  $I03 = "I03  Checksum : $ChecksumHexOld\n";
  print GCM_FILE $I03;
#Préparation de la ligne I04 
  $I04 = "I04  Type GCM : 01\n";
  print GCM_FILE $I04;
#Préparation de la ligne I05 
  $I05 = "I05  Checksum new : $ChecksumHex\n";
  print GCM_FILE $I05;
  print GCM_FILE "\n";
...
#  }
 ...
 #Remplissage du fichier GCM avec les données du fichier HEX
  if (! open(HEX_SOURCE_FILE, "$Target_Directory\\Facade_XXL_Bsp\\Release\\$HexSourcefile\.hex"))
  {
 ...
 #     system("pause");
  ...
  #on se deplace jusqua la zone a copier
